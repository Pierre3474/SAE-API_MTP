...
def afficher_etat_station(nom_station):
    donnees_station = obtenir_donnees_station(nom_station)
    if donnees_station:
        places_occupees = donnees_station['availableBikeNumber']['value']
        places_totales = donnees_station['totalSlotNumber']['value']
        etat_station = "Ouvert" if donnees_station['status']['value'] == "working" else "Fermé"
        print(f"Station: {nom_station}")
        print(f"Vélos disponibles: {places_occupees} sur {places_totales} total")
        print(f"Statut: {etat_station}")
    else:
        print(f"Station '{nom_station}' non trouvée")
...
def principal():
    ...
    while True:
        choix = input("Entrez le numéro de la station pour voir son état actuel ou 'q' pour quitter : ")
        if choix.lower() == 'q':
            break
        try:
            choix = int(choix)
            if choix == 0:
                for station in liste_stations:
                    afficher_etat_station(station)
            elif 1 <= choix <= len(liste_stations):
                afficher_etat_station(liste_stations[choix - 1])
            else:
